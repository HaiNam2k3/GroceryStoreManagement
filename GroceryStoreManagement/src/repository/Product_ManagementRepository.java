...
public class Product_ManagementRepository {
    ...
    public BillingDetail getByIdCalculate(String ID_PRODUCT) {
        ...
        try (Connection con = dbConnection.getConnection(); PreparedStatement ps = con.prepareStatement(sql)) {
            ...
            while (rs.next()) {
                bl.setId_product(rs.getString("ID_PRODUCT"));
                bl.setName_product(rs.getString("NAME_PRODUCT"));
                bl.setQuantity(rs.getInt("QUANTITY"));
                bl.setUnit_price(rs.getInt("UNIT_PRICE"));
                return bl;
            }
